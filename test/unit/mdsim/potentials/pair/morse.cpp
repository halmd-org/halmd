...
BOOST_AUTO_TEST_CASE( morse_host )
{
    ...
    // construct module
    potential_type potential(cutoff_array, epsilon_array, sigma_array, r_min_array);
    ...
    matrix_type r_min = potential.r_min_sigma();
    BOOST_CHECK(r_min(0, 0) == r_min_array(0, 0));
    BOOST_CHECK(r_min(0, 1) == r_min_array(0, 1));
    BOOST_CHECK(r_min(1, 0) == r_min_array(1, 0));
    BOOST_CHECK(r_min(1, 1) == r_min_array(1, 1));
    ...
    // expected results (r, fval, en_pot) for ε=1, σ=1, rmin=.25, rc=5σ
    ...
    // interaction AB: ε=.5, σ=2, rmin=.75, rc=5σ
    ...
    // interaction BB: ε=.25, σ=4, rmin=.5, rc=5σ
...
morse<float_type>::morse()
{
    ...
    potential = std::make_shared<potential_type>(
        cutoff_array, epsilon_array, sigma_array, r_min_array
    );
