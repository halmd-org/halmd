...
//
// Single-precision tests, discarding unused second float4 value
//

BOOST_AUTO_TEST_CASE_TEMPLATE( fixed_vector_float_int_converter_two, pair_type, float_int_types )
{
    typedef typename pair_type::first_type vector_type;
    typedef typename vector_type::value_type vector_value_type;
    typedef typename pair_type::second_type scalar_type;
    ...
    for (size_t i = 0; i < h_input.size(); ++i) {
        ...
        scalar_type v = index_to_value<scalar_type>()(i);
        tie(h_input[i], /* discarded */ h_input[i + h_input.size()]) <<= tie(u, v);
    }

    for (size_t i = 0; i < h_input.size(); ++i) {
        vector_type u;
        scalar_type v;
        tie(u, v) <<= tie(h_input[i], /* discarded */ h_input[i + h_input.size()]);
        for (size_t j = 0; j < vector_type::static_size; ++j) {
            BOOST_CHECK_CLOSE_FRACTION( u[j], index_to_value<vector_value_type>()(i, j), vector_eps );
        }
        BOOST_CHECK_EQUAL( v, index_to_value<scalar_type>()(i) );
    ...
    cuda::configure(dim.grid, dim.block);
    float_kernel<vector_type, scalar_type>::kernel.converter_two(g_input, g_output, g_u, g_v);
    cuda::thread::synchronize();
    ...
    for (size_t i = 0; i < h_output.size(); ++i) {
        vector_type u;
        scalar_type v;
        tie(u, v) <<= tie(h_output[i], /* discarded */ h_output[i + h_output.size()]);
        for (size_t j = 0; j < vector_type::static_size; ++j) {
            BOOST_CHECK_CLOSE_FRACTION( u[j], index_to_value<vector_value_type>()(i, j), vector_eps );
        }
        BOOST_CHECK_EQUAL( v, index_to_value<scalar_type>()(i) );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( fixed_vector_float_float_converter_two, pair_type, float_float_types )
{
    typedef typename pair_type::first_type vector_type;
    typedef typename vector_type::value_type vector_value_type;
    typedef typename pair_type::second_type scalar_type;
    ...
    double vector_eps = epsilon<vector_value_type>()();
    double scalar_eps = numeric_limits<scalar_type>::epsilon();
