...
static __global__ void converter_float(float4* g_input, float4* g_output, U* g_u, V* g_v)
{
    U u;
    V v;
    ...
    // pass by const reference, swap input and output
    tie(u, v) <<= static_cast<float4 const&>(g_output[GTID]);
    g_input[GTID] <<= tie(static_cast<U const&>(u), static_cast<V const&>(v));
...
template <typename U, typename V>
static __global__ void converter_double(dsfloat_ptr<float4> g_input, dsfloat_ptr<float4> g_output, U* g_u, V* g_v)
{
    U u;
    V v;
    ...
    // pass by const reference, swap input and output
    tie(u, v) <<= static_cast<halmd::tuple<float4 const&, float4 const&>>(g_input[GTID]);
