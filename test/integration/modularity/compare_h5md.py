...
"""
# Basic (two files)
python compare_h5md.py modularity-test/prod_one_instance.h5 modularity-test/prod_two_instances.h5

# Numeric only (no plots)
python compare_h5md.py file1.h5 file2.h5 --no-plot

# Custom tolerance and save figures
python compare_h5md.py file1.h5 file2.h5 --tol 1e-6 --save-prefix results/compare

# Custom groups / labels and observable list
python compare_h5md.py file1.h5 file2.h5 \
  --groups "observables/A:Group A,observables/B:Group B" \
  --observables "temperature,pressure,potential_energy"
D
...
def plot_comparison(name1, name2, grp_label, obs_list, series, show=True, save_prefix=None):
    ...
    for r, obs in enumerate(obs_list):
        ...
        # Left: overlay (prefer plotting vs time if both have same timebase; else index)
        axL = axes[r, 0]
        ...
        # Right: absolute difference (|file2 - file1|), interpolate file2 to file1's x when possible
        axR = axes[r, 1]
        ...
        axR.set_ylabel(f"|Δ {obs.replace('_', ' ').title()}|", fontsize=11)
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare H5MD observables between two files (numeric check + plots)."
    )
    ap.add_argument(
        "files",
        nargs=2,
        help="Two HDF5 files to compare (e.g., prod_one_instance.h5 prod_two_instances.h5)",
    )
    ap.add_argument(
        "--groups",
        default="observables/A:Group A,observables/B:Group B",
    ...
    ap.add_argument(
        "--observables",
        default="temperature,pressure,potential_energy,internal_energy",
        help="Comma-separated list of observables to try. Default includes 4 common ones.",
    )
    ap.add_argument(
        "--tol", type=float, default=1e-2,
        help="Absolute tolerance for np.allclose numeric check (default: 1e-2).",
    )
    ap.add_argument(
        "--no-plot", action="store_true",
        help="Skip plotting; do numeric checks only.",
    )
    ap.add_argument(
        "--save-prefix", default=None,
        help="If set, save figures to '<prefix>_GroupA.png'/'_GroupB.png' instead of only showing.",
    )
    args = ap.parse_args()
    ...
    with h5py.File(file1, "r") as f1, h5py.File(file2, "r") as f2:
        for grp_path, grp_label in groups.items():
            ...
            for obs in common:
                ...
                print_numeric_check(name1, name2, grp_label, obs, t1, v1, t2, v2, tol)
