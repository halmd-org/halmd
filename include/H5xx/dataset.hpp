...
read(H5::DataSet const& dataset, T* data, ssize_t index)
{
    ...
    H5::DataSpace dataspace(dataset.getSpace());
    if (!has_type<T>(dataset) || !has_rank<2>(dataspace)) {
        throw std::runtime_error("HDF5 reader: dataset has incompatible dataspace");
    }

    hsize_t dim[2];
    dataspace.getSimpleExtentDims(dim);
    if (dim[1] != T::static_size) {
    ...
    try {
        H5XX_NO_AUTO_PRINT(H5::Exception);
        dataset.read(data->data(), ctype<typename T::value_type>(), mem_dataspace, dataspace);
    }
    catch (H5::Exception const&) {
        throw std::runtime_error("HDF5 reader: failed to read fixed-size array data");
    }
...
read(H5::DataSet const& dataset, T* data, ssize_t index)
{
    ...
    try {
        H5XX_NO_AUTO_PRINT(H5::Exception);
        dataset.read(data->data(), ctype<typename T::element>(), mem_dataspace, dataspace);
    }
    catch (H5::Exception const&) {
