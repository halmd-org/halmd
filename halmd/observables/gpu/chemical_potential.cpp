...
#include <halmd/observables/gpu/chemical_potential.hpp>
  ...
  , logger_(logger)
{
    ...
    // construct species array in host memory first
    typedef typename particle_type::species_type species_type;
    std::vector<species_type> species(npart);
    ...
    // copy array to particle data on GPU
    set_species(*test_particle_, species.begin());
}
