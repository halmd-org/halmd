...
/**
 *  compute v[0] exp(i q·r) for each particle/wavevector pair
 *  and sum results wavevector-wise within a block
 *
 *  @returns block sums of v[0] sin(q·r), v[0] cos(q·r) for each wavevector
 *
 *  FIXME make the result vector-valued, for each component v[j] of the velocity
 */
