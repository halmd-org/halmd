...
struct wavevector
{
    ...
    typedef texture<typename current_density_mode_wrapper<dimension>::coalesced_vector_type> type;
    static type tex_;
};
...
)
{
    ...
    for (int i=0; i < nq; i++) {
        ...
        for (int j = GTID; j < npart; j += GTDIM) {
            ...
            sin_[TID] += v[0] * sin(q_r); //FIXME which element of v should be an input
            cos_[TID] += v[0] * cos(q_r);
        }
        __syncthreads();
...
template <int dimension>
current_density_mode_wrapper<dimension> const current_density_mode_wrapper<dimension>::kernel = {
    current_density_mode_kernel::wavevector<dimension>::tex_
