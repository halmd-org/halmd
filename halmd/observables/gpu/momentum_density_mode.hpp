...
#ifndef HALMD_OBSERVABLES_GPU_MOMENTUM_DENSITY_MODE_HPP
#define HALMD_OBSERVABLES_GPU_MOMENTUM_DENSITY_MODE_HPP
 ...
 *
 * @f$ \rho_{\vec q} = \sum_{i=1}^N v_i \exp(\textrm{i}\vec q \cdot \vec r_i) @f$
 *
...
public:
    ...
    momentum_density_mode(
        std::shared_ptr<particle_type const> particle
      , std::shared_ptr<particle_group_type> particle_group
      , std::shared_ptr<wavevector_type const> wavevector
      , std::shared_ptr<halmd::logger> logger = std::make_shared<halmd::logger>("momentum_density_mode")
    );

    /** Compute momentum density modes from particle group.
     *
    ...
    acquisitor(std::shared_ptr<momentum_density_mode> self)
    {
        return [=]() {
           return self->acquire();
        };
...
private:
    ...
    typedef momentum_density_mode_wrapper<dimension> wrapper_type;
    ...
    /** result for the momentum density modes */
...
#endif /* ! HALMD_OBSERVABLES_GPU_MOMENTUM_DENSITY_MODE_HPP */
