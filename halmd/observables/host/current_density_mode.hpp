...
#ifndef HALMD_OBSERVABLES_HOST_CURRENT_DENSITY_MODE_HPP
#define HALMD_OBSERVABLES_HOST_CURRENT_DENSITY_MODE_HPP
...
/**
 * Compute Fourier modes of the particle current density.
 *
...
public:
    ...
    current_density_mode(
        std::shared_ptr<particle_type const> particle
      , std::shared_ptr<particle_group_type> particle_group
      , std::shared_ptr<wavevector_type const> wavevector
      , std::shared_ptr<halmd::logger> logger = std::make_shared<halmd::logger>("current_density_mode")
    );
    ...
    acquisitor(std::shared_ptr<current_density_mode> self)
    {
        return [=]() {
           return self->acquire();
        };
...
private:
    ...
    /** result for the current density modes */
...
#endif /* ! HALMD_OBSERVABLES_HOST_CURRENT_DENSITY_MODE_HPP */
