...
void correlation::sample(uint64_t step)
{
    // trigger update of input sample(s)
    on_sample_(step);

    LOG_TRACE("[correlation] append sample(s) at step " << step);
...
//         block_sample->make_copy();
    ...
    for (unsigned int i = 0; i < interval_.size(); ++i) {
        if (step % interval_[i] == 0) {
            ...
            LOG_TRACE("[correlation] append sample(s) to blocking level " << i);
            BOOST_FOREACH(shared_ptr<block_sample_type> block_sample, block_sample_) {
            ...
            if (block_sample_.begin()->get()->full(i)) {
                // call all registered tcf modules
                // and correlate block data with first entry
                LOG_TRACE("[correlation] compute correlations at blocking level " << i);
                BOOST_FOREACH(shared_ptr<tcf_base> tcf, tcf_) {
                    tcf->compute(i);
                }
...
void correlation::finalise(uint64_t step)
{
    // iterate over all coarse-graining levels
    for (unsigned int i = 0; i < interval_.size(); ++i) {
        ...
        [
            ...
            [
                class_<correlation, shared_ptr<correlation> >("correlation_")
                    ...
                    .property("finalise", &finalise_wrapper<correlation>)
                    .property("sample", &sample_wrapper<correlation>)
                    .def("add_function", &correlation::add_function)
                    .def("add_data", &correlation::add_data)
                    .def("on_sample", &correlation::on_sample)
            ]
        ]
    ];
    return 0;
