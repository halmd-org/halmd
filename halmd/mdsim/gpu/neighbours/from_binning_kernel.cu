...
inline __device__ void update_cell_neighbours(
        ...
        for (j[1] = -1; j[1] <= 1; ++j[1]) {
            if (dimension == 3) {
                for (j[2] = -1; j[2] <= 1; ++j[2]) {
                    ...
                    update_cell_neighbours<false>(j, ncell, g_cell, r, type, n, count, g_neighbour, box_length);
                    update_cell_neighbours<false>(-j, ncell, g_cell, r, type, n, count, g_neighbour, box_length);
                }
            }
            else {
...
self:
    update_cell_neighbours<true>(j, ncell, g_cell, r, type, n, count, g_neighbour, box_length);
