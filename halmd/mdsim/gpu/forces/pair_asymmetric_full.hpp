...
#ifndef HALMD_MDSIM_GPU_FORCES_PAIR_ASYMMETRIC_FULL_HPP
#define HALMD_MDSIM_GPU_FORCES_PAIR_ASYMMETRIC_FULL_HPP
...
private:
    ...
    typedef pair_asymmetric_full_wrapper<dimension, gpu_potential_type> gpu_wrapper;
...
template <int dimension, typename float_type, typename potential_type>
pair_asymmetric_full<dimension, float_type, potential_type>::pair_asymmetric_full(
    std::shared_ptr<potential_type const> potential
  , std::shared_ptr<particle_type> particle1
  , std::shared_ptr<particle_type const> particle2
  , std::shared_ptr<box_type const> box
...
template <int dimension, typename float_type, typename potential_type>
inline void pair_asymmetric_full<dimension, float_type, potential_type>::compute_aux_()
{
    position_array_type const& position1 = read_cache(particle1_->position());
    orientation_array_type const& orientation1 = read_cache(particle1_->orientation());
    position_array_type const& position2 = read_cache(particle2_->position());
...
template <int dimension, typename float_type, typename potential_type>
void pair_asymmetric_full<dimension, float_type, potential_type>::luaopen(lua_State* L)
{
    ...
    [
        ...
        [
            namespace_("forces")
            [
                class_<pair_asymmetric_full>()
                    .def("check_cache", &pair_asymmetric_full::check_cache)
                    .def("apply", &pair_asymmetric_full::apply)
                    .scope
                    [
                    ...
                    .def_readonly("runtime", &pair_asymmetric_full::runtime_)

            , def("pair_asymmetric_full", &std::make_shared<pair_asymmetric_full,
...
#endif /* ! HALMD_MDSIM_GPU_FORCES_PAIR_ASYMMETRIC_FULL_HPP */
