...
public:
     ...
     */
    HALMD_GPU_ENABLED lennard_jones_linear(unsigned int type1, unsigned int type2)
      : pair_(
    ...
    HALMD_GPU_ENABLED tuple<float_type, float_type, float_type> operator()(float_type rr) const
    {
        ...
        float_type hvir = 576 * eps_ri6 * (ri6 - 0.25f) + r * pair_[FORCE_CUT];
...
using namespace halmd::mdsim::gpu::potentials::lennard_jones_linear_kernel;
...
template class pair_trunc_wrapper<3, lennard_jones_linear, local_r4<float> >;
template class pair_trunc_wrapper<2, lennard_jones_linear, local_r4<float> >;
