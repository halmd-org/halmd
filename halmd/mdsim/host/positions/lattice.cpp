...
void lattice<dimension, float_type>::set()
{
    ...
    typename vector<vector_type>::iterator r_it = particle_->r.begin();
    ...
    //      +---------+-----------+---------+
    //
    length = box_->length();  // start with the full box
    offset = -length / 2;
    for (unsigned int i = 0; i < dimension && r_it != particle_->r.end(); ++i) {
