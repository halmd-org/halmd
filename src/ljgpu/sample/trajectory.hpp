...
template <bool writer, typename float_type, int dimension>
class trajectory;
...
class trajectory<true, float_type, dimension>
{
public:
 ...
 */
template <typename float_type, int dimension>
class trajectory<false, float_type, dimension>
{
public:
    typedef vector<float_type, dimension> vector_type;
...
public:
    ...
    void read(std::vector<vector_type>& r, std::vector<vector_type>& v, int64_t index);
 ...
 */
template <typename float_type, int dimension>
void trajectory<true, float_type, dimension>::open(std::string const& filename, unsigned int const& npart)
{
    ...
    m_dataset[0] = root.createDataSet("t", H5xx::ctype<float_type>::type, m_ds_scalar, cparms);
}
 ...
 */
template <typename float_type, int dimension>
void trajectory<true, float_type, dimension>::close()
{
    try {
	m_file.close();
    }
 ...
 */
template <typename float_type, int dimension>
void trajectory<true, float_type, dimension>::flush()
{
    try {
 ...
 */
template <typename float_type, int dimension>
H5param trajectory<true, float_type, dimension>::attrs()
{
...
template <typename float_type, int dimension>
template <typename trajectory_sample>
void trajectory<true, float_type, dimension>::sample(trajectory_sample const& sample, float_type const& time)
{
    hsize_t dim[3];
 ...
 */
template <typename float_type, int dimension>
void trajectory<false, float_type, dimension>::open(std::string const& filename)
{
 ...
 */
template <typename float_type, int dimension>
void trajectory<false, float_type, dimension>::close()
{
    try {
	file.close();
    }
 ...
 */
template <typename float_type, int dimension>
void trajectory<false, float_type, dimension>::read(std::vector<vector_type>& r, std::vector<vector_type>& v, int64_t index)
{
    try {
	...
	dataset_r.read(r.data(), H5xx::ctype<float_type>::type, ds_mem, ds_r);
	// read particle velocities
	ds_v.selectHyperslab(H5S_SELECT_SET, count, start, stride, block);
	dataset_v.read(v.data(), H5xx::ctype<float_type>::type, ds_mem, ds_v);
    }
    catch (H5::Exception const& e) {
