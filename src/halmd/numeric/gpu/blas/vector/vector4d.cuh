 ...
 */
__device__ inline vector<float, 4> dsfloat_hi(vector<dsfloat, 4> v)
{
    vector<float, 4> w;
    w[0] = dsfloat_hi(v[0]);
    w[1] = dsfloat_hi(v[1]);
 ...
 */
__device__ inline vector<float, 4> dsfloat_lo(vector<dsfloat, 4> v)
{
    vector<float, 4> w;
    w[0] = dsfloat_lo(v[0]);
    w[1] = dsfloat_lo(v[1]);
...
__device__ inline typename boost::enable_if<boost::true_type, vector<T, 4>&>::type
operator+=(vector<T, 4>& v, vector<T, 4> const& w)
{
    v[0] += w[0];
    v[1] += w[1];
    v[2] += w[2];
