...
#ifndef MDSIM_SCAN_HPP
#define MDSIM_SCAN_HPP
...
public:
	...
	cuda::configure(blocks[0], threads, gpu::scan::boff(2 * threads * sizeof(T)), stream);
	gpu::scan::block_prefix_sum(g_array, g_array, g_sum[0], count);

	for (uint i = 1; i < blocks.size(); ++i) {
	    cuda::configure(blocks[i], threads, gpu::scan::boff(2 * threads * sizeof(T)), stream);
	    gpu::scan::block_prefix_sum(g_sum[i - 1], g_sum[i - 1], g_sum[i], blocks[i - 1]);
	}
	...
	if (blocks.size() > 1) {
		...
		gpu::scan::add_block_sums(g_sum[i - 1], g_sum[i - 1], g_sum[i], blocks[i - 1]);
	    }
	}
	if (blocks[0] > 1) {
	    cuda::configure(blocks[0], threads, stream);
	    gpu::scan::add_block_sums(g_array, g_array, g_sum[0], count);
	}
    }
