...
#ifndef MDSIM_LJFLUID_GPU_NBR_HPP
#define MDSIM_LJFLUID_GPU_NBR_HPP
...
void ljfluid_gpu_impl_neighbour<dimension>::cell_occupancy(float_type value)
{
	...
	cuda::copy(ncell, gpu::ljfluid_gpu_neighbour::ncell);
	cuda::copy(cell_length_, gpu::ljfluid_gpu_neighbour::r_cell);
	cuda::copy(std::pow(cell_length_, 2), gpu::ljfluid_gpu_neighbour::rr_cell);
	cuda::copy(nbl_size, gpu::ljfluid_gpu_neighbour::nbl_size);
    }
    catch (cuda::error const&) {
    ...
    uint level = std::min((dimension == 3) ? 10.f : 16.f, ceilf(logf(box_) / M_LN2));
...
void ljfluid_gpu_impl_neighbour<dimension>::threads()
{
	...
	cuda::copy(uint(dim_.threads()), gpu::ljfluid_gpu_neighbour::nbl_stride);
    }
    catch (cuda::error const&) {
	...
	uint max_blocks = (16 * 512) / (threads * gpu::radix::BUCKETS_PER_THREAD / 2);
...
void ljfluid_gpu_impl_neighbour<dimension>::restore(trajectory_visitor visitor)
{
	...
	gpu::ljfluid_gpu_neighbour::init_tags(g_part.tag);
...
{
    cuda::configure(dim_.grid, dim_.block, stream);
    gpu::ljfluid_gpu_neighbour::mdstep(g_part.r, g_part.v, g_part.f, g_nbl, g_part.en, g_part.virial);
}
...
void ljfluid_gpu_impl_neighbour<dimension>::maximum_velocity(cuda::stream& stream)
{
    ...
    gpu::ljfluid_gpu_neighbour::maximum_velocity(g_part.v, g_part.v_max);
...
void ljfluid_gpu_impl_neighbour<dimension>::assign_cells(cuda::stream& stream)
{
    ...
    cuda::configure(dim_.grid, dim_.block, stream);
    gpu::ljfluid_gpu_neighbour::compute_cell(g_part.r, g_aux.cell);

    // generate permutation array
    cuda::configure(dim_.grid, dim_.block, stream);
    gpu::ljfluid_gpu_neighbour::gen_index(g_aux.idx);
    radix_(g_aux.cell, g_aux.idx, stream);

    // compute global cell offsets in sorted particle list
    cuda::memset(g_aux.offset, 0xff);
    cuda::configure(dim_.grid, dim_.block, stream);
    gpu::ljfluid_gpu_neighbour::find_cell_offset(g_aux.cell, g_aux.offset);

    // assign particles to cells
    cuda::configure(dim_cell_.grid, dim_cell_.block, stream);
    gpu::ljfluid_gpu_neighbour::assign_cells(g_aux.cell, g_aux.offset, g_aux.idx, g_cell);
}
...
void ljfluid_gpu_impl_neighbour<dimension>::hilbert_order(cuda::stream& stream)
{
    ...
    cuda::configure(dim_.grid, dim_.block, stream);
    gpu::ljfluid_gpu_neighbour::order_particles(g_aux.idx, g_sort.r, g_sort.R, g_sort.v, g_sort.tag);
...
#endif /* ! MDSIM_LJFLUID_GPU_NBR_HPP */
