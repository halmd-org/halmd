...
#ifndef MDSIM_LJFLUID_GPU_CELL_HPP
#define MDSIM_LJFLUID_GPU_CELL_HPP

#include "H5param.hpp"
#include "H5xx.hpp"
#include "exception.hpp"
#include "gpu/ljfluid_cell_glue.hpp"
...
{
    // no particle-dependent memory allocations
}
...
void ljfluid_gpu_impl_cell<dimension>::restore(trajectory_visitor visitor)
{
	...
	gpu::ljfluid_gpu_cell::assign_cells(g_part.R, g_part.r, g_part.tag);
...
void ljfluid_gpu_impl_cell<dimension>::lattice()
{
	...
	cuda::configure(dim_cell_.grid, dim_cell_.block, stream_);
	gpu::ljfluid_gpu_cell::assign_cells(g_part_buf.r, g_part.r, g_part.tag);
	...
	cuda::configure(dim_cell_.grid, dim_cell_.block, stream_);
	gpu::ljfluid_gpu_cell::mdstep(g_part.r, g_part.v, g_part.f, g_part.tag, g_part.en, g_part.virial);
...
void ljfluid_gpu_impl_cell<dimension>::update_cells(cuda::stream& stream)
{
    ...
    gpu::ljfluid_gpu_cell::update_cells(g_part.r, g_part.R, g_part.v, g_part.tag, g_part_buf.r, g_part_buf.R, g_part_buf.v, g_part_buf.tag);
}
...
#endif /* ! MDSIM_LJFLUID_GPU_CELL_HPP */
