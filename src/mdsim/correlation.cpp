 ...
 */
void correlation::steps(uint64_t const& value, double const& timestep)
{
    // set total number of simulation steps
    m_steps = value;
 ...
 */
void correlation::time(double const& value, double const& timestep)
{
    // set total simulation time
    m_time = value;
    LOG("total simulation time: " << m_time);
 ...
 */
void correlation::q_values(unsigned int const& n, double const& box)
{
 ...
 */
void correlation::sample(std::vector<hvector> const& r, std::vector<hvector> const& v, uint64_t const& step, bool& flush)
{
