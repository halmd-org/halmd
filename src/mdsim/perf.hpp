 ...
 */
typedef boost::array<perf_accumulator<double>, 3> perf_counters;
...
void perf<dimension, T>::open(std::string const& filename)
{
	...
	m_dataset[1] = node.createDataSet("queue", m_tid, ds, cparms);
	// CPU ticks for phase space sampling
	m_dataset[2] = node.createDataSet("sample", m_tid, ds, cparms);
    }
    catch (H5::FileIException const& e) {
...
{
    LOG("mean CPU time for MD simulation step: " << m_times[0]);
    LOG("mean CPU time for event queue processing: " << m_times[1]);
    LOG("mean CPU time for phase space sampling: " << m_times[2]);
